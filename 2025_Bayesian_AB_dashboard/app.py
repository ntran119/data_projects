...
# ── Row 1: Distribution donut + Decision day histogram ──
    ...
    for key in ordered:
        ...
        if len(sub):
            fig_day.add_trace(go.Histogram(
                x=sub, name=LABELS[key],
                marker_color=COLORS[key],
                nbinsx=p["max_days"], opacity=0.85,
            ))
    fig_day.update_layout(
        barmode="stack",
        xaxis_title="Day", yaxis_title="Simulations",
...
# ── Row 2: Posterior distributions + Lift distribution ──
col_c, col_d = st.columns(2)

with col_c:
    st.subheader("Posterior distributions (avg cumulative data)")
    ...
    for key in ordered:
        ...
        if len(sub):
            fig_lift.add_trace(go.Histogram(
                x=sub, name=LABELS[key],
                marker_color=COLORS[key],
                opacity=0.80, nbinsx=40,
            ))
    fig_lift.add_vline(
        x=p["mde"] * 100, line_dash="dash", line_color="gray",
        annotation_text=f"MDE ({p['mde']*100:.1f}%)",
    )
    fig_lift.add_vline(x=0, line_color="black", line_width=1)
    fig_lift.update_layout(
        barmode="overlay",
        xaxis_title="Mean lift at decision (pp)", yaxis_title="Simulations",
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        margin=dict(t=40, b=40, l=40, r=10), height=300,
    )
...
# ── Row 3: P(B>A) scatter + P(meaningful) scatter ──
...
with col_e:
    ...
    fig_prob = px.scatter(
        df_plot.reset_index(), x="index", y=df_plot["prob_b_better"].values * 100,
        color="decision_label",
        color_discrete_map={LABELS[k]: COLORS[k] for k in COLORS},
        labels={"index": "Simulation #", "y": "P(B > A) %", "decision_label": "Decision"},
        height=280,
    )
    fig_prob.add_hline(
        y=50, line_dash="dash", line_color="gray",
        annotation_text="50% (no evidence either way)",
    )
    fig_prob.update_layout(showlegend=False, margin=dict(t=10, b=40, l=40, r=10))
    st.plotly_chart(fig_prob, use_container_width=True)

with col_f:
    st.subheader("P(lift > MDE) — final value per simulation")
    fig_pmean = px.scatter(
        df_plot.reset_index(), x="index", y=df_plot["prob_meaningful"].values * 100,
        color="decision_label",
        color_discrete_map={LABELS[k]: COLORS[k] for k in COLORS},
        labels={"index": "Simulation #", "y": "P(lift > MDE) %", "decision_label": "Decision"},
        height=280,
    )
    fig_pmean.add_hline(
        y=p["prob_threshold"] * 100, line_dash="dash", line_color="#3B6D11",
        annotation_text=f"Ship threshold ({p['prob_threshold']:.0%})",
    )
    fig_pmean.add_hline(
        y=10, line_dash="dash", line_color="#A32D2D",
        annotation_text="Futility threshold (10%)",
    )
    fig_pmean.update_layout(showlegend=False, margin=dict(t=10, b=40, l=40, r=10))
    st.plotly_chart(fig_pmean, use_container_width=True)
    ...
    for label, color in zip(label_order, color_seq):
        ...
        if len(sub):
            fig_box.add_trace(go.Box(
                y=sub, name=label,
                marker_color=color, boxmean="sd", line_width=1.5,
            ))
    fig_box.update_layout(
        xaxis_title="Experiment outcome", yaxis_title="Day decision was made",
        showlegend=False, margin=dict(t=10, b=40, l=40, r=10), height=320,
    )
...
with col_h:
    ...
    fig_strip.update_layout(
        xaxis=dict(
            tickmode="array", tickvals=list(range(len(label_order))),
            ticktext=label_order, title="Experiment outcome",
        ),
...
with col_i:
    fig_boundary = px.scatter(
        df_plot, x="prob_meaningful", y="prob_b_better",
        color="decision_label",
        color_discrete_map={LABELS[k]: COLORS[k] for k in COLORS},
        opacity=0.55,
        labels={
    ...
    fig_boundary.add_vline(
        x=0.10, line_dash="dash", line_color="#A32D2D",
        annotation_text="Futility (10%)",
...
with col_j:
    ...
    fig_kde.update_layout(
        ...
        showlegend=True,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        margin=dict(t=40, b=40, l=40, r=10), height=340,
    )
    ...
    for label, color in zip(label_order, color_seq):
        ...
        if len(sub):
            fig_ci_box.add_trace(go.Box(
                ...
                boxmean="sd", line_width=1.5, opacity=0.85, boxpoints=False,
            ))
    for i, (label, color) in enumerate(zip(label_order, color_seq)):
        sub = df_plot[df_plot["decision_label"] == label]
        if len(sub):
    ...
    for label, color in zip(label_order, color_seq):
        ...
        if len(sub):
            fig_ci_hist.add_trace(go.Histogram(
                ...
                opacity=0.80, nbinsx=30,
            ))
    fig_ci_hist.update_layout(
        barmode="stack",
        xaxis_title="CI width", yaxis_title="Count",
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        margin=dict(t=40, b=40, l=40, r=10), height=320,
    )
